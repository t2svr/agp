...
where Self::ValueType: Clone + 'static , Self::IdType: Clone + Eq + Hash + Display
{
    ...
    fn start(&mut self) -> Result<bool, MemError> {
        ...
        else { Err(MemError::new("Mem start failed.")) }
    }
}
...
pub enum OperationType {
    ObjAdd,
    ObjAddBatch,
    ObjRemove,      // 在当前膜内 移除 一个id为target_id的对象p_obj
    ObjOut,         // 向 膜外 id为target_id的对象传递对象p_obj, 可以跨越多层 
    ObjIn,          // 向 膜内 id为target_id的对象传递对象p_obj, 可以跨越多层  
    RuleAdd,
    RuleAddBatch,
    MemAdd,         // 在 膜内 添加一个id为target_id的膜p_mem
    MemRemove,
    MemAttachOutter,
    MemAttachInner,
    Stop
